...
impl LookAngles {
    ...
    pub fn assert_not_looking_up(&self) {
        let is_looking_up = relative_eq!(self.unit_vector().dot(DVec3::Y).abs(), 1.0);
...
fn yaw_and_pitch_from_vector(v: DVec3) -> (f64, f64) {
    debug_assert_ne!(v, DVec3::ZERO);

    let y = DVec3::Y;
    let z = DVec3::Z;

    let v_xz = DVec3::new(v.x, 0.0, v.z);
...
fn unit_vector_from_yaw_and_pitch(yaw: f64, pitch: f64) -> DVec3 {
    let ray = DMat3::from_rotation_y(yaw) * DVec3::Z;
    let pitch_axis = ray.cross(DVec3::Y);

    DMat3::from_axis_angle(pitch_axis, pitch) * ray
}
...
mod tests {
    ...
    #[test]
    fn test_yaw_only() {
        let (yaw, pitch) = yaw_and_pitch_from_vector(DVec3::new(1.0, 0.0, 0.0));
    ...
    #[test]
    fn test_pitch_only() {
        let (yaw, pitch) = yaw_and_pitch_from_vector(DVec3::new(0.0, 1.0, 0.0));
        assert_relative_eq!(yaw, 0.0);
    ...
    fn test_yaw_and_pitch() {
        let (yaw, pitch) = yaw_and_pitch_from_vector(DVec3::new(0.5f64.sqrt(), 1.0, 0.5f64.sqrt()));
        assert_relative_eq!(yaw, PI / 4.0, epsilon = 1e-6f64);
        assert_relative_eq!(pitch, PI / 4.0);

        let (yaw, pitch) =
            yaw_and_pitch_from_vector(DVec3::new(-0.5f64.sqrt(), -1.0, 0.5f64.sqrt()));
        assert_relative_eq!(yaw, -PI / 4.0, epsilon = 1e-6f64);
        assert_relative_eq!(pitch, -PI / 4.0);
    }
}
